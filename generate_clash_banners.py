...
BANNER_W, BANNER_H = 1200, 400   # wide fight-poster banner — sized for the MATCH CARD header
# (3:1). These are used ONLY as card-top clash headers (app.js clashBannerPath); they are NOT
# in the hero rotation, so this card-friendly aspect is the right native size. The two owners
# sit on the left/right halves with a flare between, so the card shows the banner whole.
